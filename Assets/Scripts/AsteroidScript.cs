...
public class AsteroidScript : MonoBehaviour
{
    ...
    void OnEnable(){
        ...
        xTarget = 1.5f*((transform.position.x - ship.transform.position.x) + Random.Range(-1000f, 1001f));
        // Get the ship's y position and add a random offset
        yTarget = 1.5f*((transform.position.y - ship.transform.position.y) + Random.Range(-1000f, 1001f));
        // Get the ship's z position and add a random offset
        zTarget = 1.5f*((transform.position.z - ship.transform.position.z) + Random.Range(-1000f, 1001f));
